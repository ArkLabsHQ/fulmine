...
var fulmineRuntimeMetrics = []string{
...
func collectGoRuntimeMetrics(ctx context.Context) {
	...
	samples := make([]metrics.Sample, 0, len(fulmineRuntimeMetrics))
	for _, n := range fulmineRuntimeMetrics {
...
// fulmineMetricName converts e.g. "/cpu/classes/user:cpu-seconds" to "fulmine_cpu_classes_user_cpu-seconds"
func fulmineMetricName(name string) string {
	clean := strings.ReplaceAll(name, "/", "_")
	clean = strings.ReplaceAll(clean, ":", "_")
	for strings.HasPrefix(clean, "_") {
		clean = clean[1:]
	}
	return "fulmine_" + clean
}

type fulmineInstruments struct {
...
func initFulmineRuntimeInstruments(m metric.Meter) (*fulmineInstruments, error) {
	inst := &fulmineInstruments{
...
func collectInstruments(inst *fulmineInstruments) []metric.Observable {
